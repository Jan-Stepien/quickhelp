...
export const metadata: Metadata = buildMetadata({
  ...
  keywords: ["JSON formatter", "JSON validator", "pretty print JSON", "JSON minifier", "JSON beautifier", "JSON lint", "format JSON online"],
});
...
export default function JsonFormatterPage() {
  ...
  const breadcrumb = buildBreadcrumbJsonLd([
    ...
    { name: "JSON Formatter", url: `${baseUrl}/json-formatter` },
  ]);
  const content = jsonFormatter.content;
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-10">
      ...
      <header className="space-y-2">
        ...
        <p className="text-lg text-muted-foreground">{jsonFormatter.summary}</p>
      </header>
      ...
      <footer className="border-t border-border pt-6 text-xs text-muted-foreground">
        <p>
          <strong>API:</strong>{" "}
          <code className="font-mono">POST {baseUrl}/api/json-formatter</code> — JSON in, JSON out.
          See{" "}
